...
def pairs(cards):
    ...
    for c in cardType:
        if c in newcards:
            newcards.remove(c)
    return [sortByValue(list(cardType)), sortByValue(list(newcards))]

def three(cards):#assume 5 cards
    cardType = set()
    newcards = sortByValue(cards)
    for i in range(len(newcards)):
        if i < len(cards)-2:
            if match([newcards[i], newcards[i+1], newcards[i+2]]):
    ...
    if len(cardType)>0:
        ...
        newcards = set(newcards)
        newcards.remove(c)
        return [c]+sortByValue(list(newcards))
    else: return []
...
def get_hand_val(hand):
    ...
    if  royalFlush(cards):
        ...
        return straight_flush_val(cards)
    f = four(cards)#bind four fo a kind check
    if f[0]:#len(f)!=0:
        #print("4 of a kind")
        return four_of_a_kind_val(f[0], f[1])#return
    fh = fullHouse(cards)
    if fh:
        #print("full house")
